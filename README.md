...
[![Tests](https://img.shields.io/badge/tests-154%20passing-brightgreen?style=flat-square)](https://github.com/padosoft/laravel-flow-admin/actions)
...
- 🧪 **Battle-tested** — 154 PHPUnit tests, 20 Playwright scenarios (60 runs across Chromium / Firefox / WebKit — 54 pass, 6 skipped: 3 visual-gated + 1 WebKit drag-and-drop limitation + 2 cross-browser click/drag limitations on one node-deletion scenario).
...
composer test                 # PHPUnit — 154 tests, 800 assertions
npm run lint                  # ESLint flat config
npm run build                 # Vite build verification
npm run test:e2e              # Playwright on chromium + firefox + webkit
```

Latest local run: **154 tests / 800 assertions / 54 E2E runs passed** (20 Playwright scenarios × 3 browsers, 6 skipped: 3 visual-gated + 1 WebKit drag-and-drop limitation + 2 cross-browser limitations on one node-deletion scenario).
