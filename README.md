...
[![Tests](https://img.shields.io/badge/tests-230%20passing-brightgreen?style=flat-square)](https://github.com/padosoft/laravel-flow-admin/actions)
...
- 🧩 **Flow Studio canvas** — React + `@xyflow/react` visual graph: read-only view of a flow's published version, plus a full drag-and-drop editor (palette from the node catalog, typed-connection validation, provenance/taint warnings, node inspector, save-as-draft) gated by your `ActionAuthorizer`.
- 🛑 **Provenance warnings on the canvas** — core rejects a graph where untrusted data can reach a port that decides what gets executed, so wiring an LLM into a shell command was already impossible to publish. The canvas now says so *while you draw it*: the offending wire turns **amber** (not red — the ports connect fine, it is the data that is wrong, usually several hops upstream), Save is blocked, and the message names the whole route: `llm.summary -> format.out -> shell.command`. The alternative was a 422 after pressing Save, naming node ids you then had to hunt for on the canvas.
...
- 🧪 **Battle-tested** — 246 PHPUnit tests, 36 Playwright scenarios (99 runs across Chromium / Firefox / WebKit — 93 pass, 6 skipped: 3 visual-gated + 1 WebKit drag-and-drop limitation + 2 cross-browser click/drag limitations on one node-deletion scenario). No test is skipped to hide a failure, and the e2e server no longer runs PHP's **experimental forking** built-in server: a forked worker segfaulting mid-request returned an empty response the browser reported as `net::ERR_EMPTY_RESPONSE`, and no retry can recover a response that was never written. The background `/flow/api/live` heartbeat — which no test needs — is switched off for e2e instead, leaving a single worker enough to serialise what remains. See `scripts/serve-testbench.mjs`.
...
composer test                 # PHPUnit — 230 tests, 1045 assertions
npm run lint                  # ESLint flat config
npm run build                 # Vite build verification
npm run test:e2e              # Playwright on chromium + firefox + webkit
```

Latest local run: **230 tests / 1045 assertions / 93 E2E runs passed** (33 Playwright scenarios × 3 browsers, 6 skipped: 3 visual-gated + 1 WebKit drag-and-drop limitation + 2 cross-browser limitations on one node-deletion scenario).
