...
**How to apply:** the config env a `testbench serve` E2E needs at RUNTIME (DB connection/name, and anything the served routes read) must be declared in **`testbench.yaml`'s `env:` block**, which the served app loads itself — NOT relied upon via the serve script's spawned-process env, which the `php -S` grandchild may or may not inherit depending on OS. Set `DB_CONNECTION`/`DB_DATABASE` there so the served app and the migration target the same file on every platform. For a relative sqlite `DB_DATABASE`, Laravel's `SQLiteConnector` resolves it as `realpath($path) ?: realpath(base_path($path))` — so it resolves against the process CWD (the serve cwd = repo root) and REQUIRES the file to already exist when the first connection opens (it does: migrate runs before the server starts). `testbench.yaml`'s `env:` block does NOT leak into the PHPUnit suite (that's driven by `phpunit.xml`'s own `testing`/`:memory:` env), so adding DB vars there is safe for unit/feature tests. WAL journal mode + `busy_timeout` (via `scripts/enable-wal.php`) is kept as genuine defense now that writes actually hit the shared file concurrently with the `/flow/api/live` poll's reads — but it was NOT the fix here.
