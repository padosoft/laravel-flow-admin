...
### A single-browser E2E job failing (a different browser each push) is the single-threaded `testbench serve` flaking, not a code bug
Across three PR-42 CI rounds, the E2E matrix failed on a DIFFERENT browser each time (chromium round 2, webkit round 3) while the other two browsers — running the IDENTICAL specs as separate jobs, each with its own `serve-testbench.mjs` server — passed, and the full local run passed on all three. The failure signature: ONE test times out (~6s), then every subsequent test (including unrelated specs) fails fast (~200ms) — the classic "the single-process `php artisan serve` stalled/died mid-job and every later `page.goto` instantly fails" cascade, not an assertion failure. **How to apply:** when a browser-parallel E2E matrix fails on a single shard with a timeout-then-cascade pattern while the sibling shards are green, treat it as infra flakiness — re-run (a fresh push re-runs all shards), don't "fix" your code. A real fix is server-side (a concurrent server, or Playwright `webServer.reuseExistingServer`/retry tuning), tracked separately. Verify by running the full `npm run test:e2e` locally across all three browsers before concluding.

### A time-window-boundary KPI test is inherently flaky (seeds `now - P1D`, query re-evaluates `now`)
`EloquentReadModelTest::test_kpi_window_boundary_does_not_double_count_a_run` seeds a run at exactly `now - P1D` and asserts it lands in exactly one KPI window; but the KPI query computes its OWN `now` microseconds later, so the boundary run occasionally flips windows and the test fails ~1 run in N. It is unrelated to E-PR6 (untouched file) and passes on re-run. **How to apply:** don't chase an intermittent failure in an untouched time-boundary test as if your diff caused it — re-run to confirm flakiness; a real fix means seeding just INSIDE a window (e.g. `now - P1D + 1s`) or injecting a fixed clock, tracked separately.
