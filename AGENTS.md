...
- **Public surface:** consume only `Padosoft\LaravelFlow\Dashboard\*`, `Padosoft\LaravelFlow\Contracts\DefinitionRepository`, `Padosoft\LaravelFlow\Node\NodeRegistry`/`NodeDefinition`/`Attributes\*`/`PortType`, and `Padosoft\LaravelFlow\Graph\GraphSerializer`/`GraphValidator`/`StoredDefinition`/`Exceptions\InvalidGraphException` (the named exceptions — all `@api`-stable, needed for declared graph/step-count reads, the Studio canvas's node catalog, and the Studio editor's structural/semantic graph validation on save (E-PR3), none of which `Dashboard\*` has a primitive for), and the documented action API. Never reference `Persistence`/`Models`/`Queue`/`Jobs`/`Console` namespaces of the engine.
