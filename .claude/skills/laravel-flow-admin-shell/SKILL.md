...
- `Padosoft\LaravelFlow\Executor\DryRun\DryRunPlanner` / `ExecutionPlan` / `CostEstimate` — another named exception, `@api`-stable, the ONLY `Executor\*` classes the admin may consume: the Studio editor's dry-run button statically plans the current graph (Kahn-wave execution order + per-node/total cost) with zero writes and no handler execution (see Macro E / E-PR7). No approved `Dashboard\*`/facade seam returns the plan+cost, so this named `@api` planner is used directly.
